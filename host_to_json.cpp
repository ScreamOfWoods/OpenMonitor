...
void HostToJson::addAddressesToJson()
{
    vector<string> ip = host_machine.getIpAddresses();
    vector<string> mac = host_machine.getMacAddresses();

    for(uint32_t i = 0; i < ip.size(); i++) {
        jdoc["ip_addresses"].push_back("'"+ ip.at(i) +"'");
    }

    for(uint32_t i = 0; i < mac.size(); i++) {
        jdoc["mac_addresses"].push_back("'"+ ip.at(i) +"'");
    }
}
...
void HostToJson::addProcessesToJson()
{
    ...
    for(uint32_t i = 0; i < procs.size(); i++) {
        ...
        jdoc["processes"].push_back({
                ...
                {"state", state + procs.at(i).getState() + "'"},
